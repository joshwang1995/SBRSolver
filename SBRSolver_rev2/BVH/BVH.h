...
public:
	...
	bool RayIntersects(int index, const Vec3& rayOrg, const Vec3& rayDir, HitInfo& info);
...
void BVH<T>::ConstructBVHInternal(std::vector<T*>& objects, const int nodeIdx)
{
	...
	for (int axis = 0; axis < 3; axis++)
	{
		...
		(
			...
			{
				// lambda expression
				//	sort from smallest x,y,z values to biggest
...
{
	if (nodes[index].IntersectsAABB(rayOrg, rayDir))
	{
		if (nodes[index].children[0] != -1)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				bool result = RayIntersects(nodes[index].children[i], rayOrg, rayDir, info);
				if (result != false)
				{
					childResult = result;
				}
